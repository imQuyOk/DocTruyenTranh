                            ...
                            {comics?.[slug]?.map((comic, index) => (
                                <li key={index} className='lg:w-[12.5%] md:w-[16.66667%] sm:w-[25%] mobile:w-[50%] flex-shrink-0 px-[8px] transition-all relative overflow-hidden group hover:translate-y-[-8px] duration-300'>
                                    <Link to={`/read/${slug}/${comic?.data?.item?._id}`}>
                                        ...
                                        <h4 className="mt-[12px] transition-all duration-300 group-hover:text-[#e4d71c] font-[600] dark:text-[#fff]">
                                            {`Chương ${comic?.data?.item?.chapter_name}`}
                                        </h4>
                                    </Link>
                                    <button
                                        title="Xoá chương khỏi lịch sử xem"
                                        onClick={() => handleDeleteComicOutHistory(slug, index)} className="absolute top-0 right-[8px] text-base py-[6px] px-[12px] bg-[rgba(221,189,23,1)] text-[#fff] rounded-bl-[8px] rounded-tr-[8px] duration-300 hover:px-[16px] hover:py-[12px] hover:text-xl">
