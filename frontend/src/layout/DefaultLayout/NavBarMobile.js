    ...
    return (
        <div
            ...
            <div ref={modalRef} className="relative w-[80%] bg-[#fff] dark:bg-[#2f2f2f] dark:text-[#fff rounded-r-[16px] h-full left-0 animate-slide-in p-[16px]">
                ...
                <div className="h-[40px] flex items-center px-[8px] py-[4px] border-2 border-solid border-[#ccc] rounded-[8px] transition-all w-full focus-within:border-[#e4d71c] mt-[64px]">
                    <input
                        value={valueSearch}
                    ...
                    />
                    <NavLink onClick={handleCloseModal} className={`px-[8px] transition-all dark:text-[#fff] hover:text-[#e4d71c] ${valueSearch !== '' ? 'pointer-events-auto' : 'pointer-events-none'}`} to={`/search/${valueSearch}`}>
                        <i className="text-inherit fa-solid fa-magnifying-glass"></i>
                    </NavLink>
                </div>
                <ul className="mt-[12px]">
                    <li onClick={handleCloseModal}>
                        <NavLink to='/' className={`block text-lg hover:text-[#e4d71c] transition-all py-[8px] ${pathname === '/' ? 'text-[#e4d71c] font-[900]' : 'text-[#000] dark:text-[#fff]'}`}>Trang chủ</NavLink>
                    </li>
                    <li onClick={handleCloseModal}>
                        <NavLink to='/detail/danh-sach/truyen-moi' className={`block text-lg hover:text-[#e4d71c] transition-all py-[8px] ${pathname === '/detail/danh-sach/truyen-moi' ? 'text-[#e4d71c] font-[900]' : 'text-[#000] dark:text-[#fff]'}`}>Truyện mới nhất</NavLink>
                    </li>
                    <li onClick={handleCloseModal}>
                        <NavLink to='/archive' className={`block text-lg hover:text-[#e4d71c] transition-all py-[8px] ${pathname === '/archive' ? 'text-[#e4d71c] font-[900]' : 'text-[#000] dark:text-[#fff]'}`}>Kho lưu trữ</NavLink>
                    </li>
                    <li onClick={handleCloseModal}>
                        <NavLink to='/history' className={`block text-lg hover:text-[#e4d71c] transition-all py-[8px] ${pathname === '/history' ? 'text-[#e4d71c] font-[900]' : 'text-[#000] dark:text-[#fff]'}`} >Lịch sử đã xem</NavLink>
                    </li>
                    <li>
