                        ...
                        <ul className="flex flex-wrap gap-[12px] p-[16px]">
                            {data?.data?.items.map((category, index) => (
                                <li
                                    ...
                                    <Link
                                        className={`block p-[8px]  rounded-[8px] transition-all hover:bg-[#e4d71c] hover:text-[#fff] text-center cursor-pointer select-none ${pathname === `/detail/the-loai/${category?.slug}` ? 'text-[#e4d71c] font-[900]' : 'text-[#000] dark:text-[#fff]'}`}
                    ...
                    {data?.data?.items.map((category, index) => (
                        <li key={index}>
                            <Link
                                ...
                                className={`py-[8px] block ${pathname === `/detail/the-loai/${category?.slug}` ? 'text-[#e4d71c] font-[900]' : 'text-[#000] dark:text-[#fff]'}`}
                                to={`/detail/the-loai/${category?.slug}`}>
                                {category?.name}
                            </Link>
                        </li>
